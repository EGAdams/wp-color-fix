...
function mcba_init( root, www, ajax ) {
    ...
    jQuery( '#mcba_mockup' ).load( function () {
    ...
    // jQueryjQuery = jQuery(this)[0].contentWindow.jQuery;
    //MCBA = jQuery(this)[0].contentWindow.MCBA;
    // buildPage(); moved to inside load script on feb 9.  when MCBA.template is null, widgets don't show
    MCBA.onPageLoad = onPageLoad;
    MCBA[ "www_url" ] = www_url;
...
function onDragOver( event, ui ) {
    if ( jQuery( this ).attr( 'id' ) === "mcba_item_overview" ) {
        if ( ui.draggable.data( 'from' ) !== "toolbox" ) {
        } else {
...
function onPageLoad() {
    updateItemOverview();
    //jQuery( '#mcba_mockup' ).load( '<html><body><div>Hello!line 167 in mcba_functions.js</div></body></html>' ); 
...
function buildPage() {
    ...
    jQuery( '#mcba_item_overview' ).droppable( { 'over': onDragOver, 'out': onDragOut, 'drop': onDragDrop } );
    // var JQUERY_UI_VERSION = "1.10.4";
    // var JQUERY_UI_VERSION = "1.13.0";
    // console.log( "loading jquery ui version: " + JQUERY_UI_VERSION );
    // MCBA.loadScript( "https://code.jquery.com/ui/" + JQUERY_UI_VERSION + "/jquery-ui.js", function( result ) {
    // console.log( "result: " + result );
    //     jQuery( "#mcba_item_container" ).append( theScript );
    //     setTimeout( function () {
    //         console.log( "sortable was called here."); }, 10000 );
    //});
}
