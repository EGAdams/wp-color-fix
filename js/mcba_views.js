/** 
 * .../wp-content/plugins/MCBA-Wordpress/js/mcba_views.js
 */
...
function Anchor( attrs ) {
    ...
    this.anchor = jQuery( document.createElement( 'a' ) )
        ...
        .css( {
            ...
            // 'backgroundImage': 'url( ' + attrs.img + ' )' showing blank squares
        });
...
function CustomButton( attrs ) {
    ...
    if ( typeof attrs.action !== 'undefined' ) {
        if ( attrs.action == "accept_background_location" ) {
            ...
            this.element.click( function () { window.location.href = attrs.action; }); }
    }
...
function PropertyObjectFactory(parent) {
    ...
    this.createPropertyObject = function(propertyKey) { 
        var object_name = self.getPropertyObjectName(propertyKey);
        var constructor = window[object_name];
        if(!constructor)
            return null;
        var object = new constructor(self.parentObject);
        return object;
    };
...
function ActionSelectElement() {
    ...
    this.createElement = function () {
        ...
        var signoff = Object();
        signoff.value = "mcba://signoff";
        signoff.text = "Sign Off";
        options.push( signoff );
...
function Block( /* PageItem || String */ data ) {
    ...
    blockLogger = LoggerFactory.createLogger( "BlockLogger" );
    ...
    if ( typeof data === "string" ) {  /* create the widget toolbox in the right column */
        ...
        if ( this.item.template === undefined ) {
            blockLogger.logUpdate( "*** ERROR: Block: " + this.item.type + " has no template. ***" );
        } else {
            if ( this.item.template[ 'has-options' ] != false ) {
                this.menu = new BlockMenu( this );
            } else {
                if ( this.item.template[ 'has-options' ] == false ) {
                    cssClass = 'mcba_block_no_action ';
                } 
            }
        }
    ...
    this.element = jQuery( document.createElement( 'div' ))
        ...
        .css({
            'background': ( block && block.item && block.item.template && block.item.template.color ) ? block.item.template.color : '#e0e0e0'
        })
        .hover(
            /* Hover over */
            function () {
...
function checkActions( block ) {
    if( block.item.type != "Buttonrow" ) { 
        console.log( "no actions, returing true..." ); return true; }
    else { return true; }

    // Get the div element - assuming it's uniquely identified or the first of its type
    var targetDiv = block.element[ 0 ]; // Adjust the class name as needed

    // Find all label elements within the target div that contain the text "Button Action"
    var labels = targetDiv.querySelectorAll('label');

    // Iterate over each label and find the select element within the same parent container
    labels.forEach(function(label) {
        if (label.textContent.includes("Button Action")) {
            // Find the parent container of the label
            var container = label.parentElement;
            
            // Search within this container's parent element for the select element
            var selectElement = container.parentElement.querySelector( 'select' );
            if (selectElement) {
                console.log('ID:', selectElement.id, 'Selected Value:', selectElement.value);
            } else {
                console.log('Select element not found for label:', label.textContent);
            }
        }
    });
}
...
function BlockList( pageitems ) {
    ...
    this.addBlock = function ( block ) { // MARK: add Block
        this.blocks.push( block );
        if ( block instanceof Block ) { block = block.element; } // pull out the element...
        this.element.append( jQuery( document.createElement( 'li' )).append( block )); // add it...
        this.count++;
        this.length = this.count;
    };
    ...
    for ( var i = 0; i < pageitems.length; i++ ) {
         ...
         */
        if ( block.menu != undefined ) { pageitems[ i ].children = block.menu.children; }
        jQuery( block.element ).attr( 'id', jQuery( block.element ).attr( 'id' ) + i );
        if ( checkActions( block )) { this.addBlock( block );
        } else {
            error = "*** ERROR: action in app_config does not match selected action. ***";
            console.log( error ); }}

    this.hideAllMenus = function () {
        for ( var i = 0; i < this.blocks.length; i++ ) {
            if ( this.blocks[ i ].menu != null ) { this.blocks[ i ].menu.hide(); }}}} // end class
/*
#######
#        #    #  #####
#        ##   #  #    #
...
function HTMLItem( item ) {
    ...
    //                 'float': 'left',
    //             })
    //             .append( "html_item" /* self.model.display */ )
    //     )

    //     /* Create the input box */
    //     .append(
    //         jQuery( document.createElement( 'textarea' ))
    //             .text( "html_model"  /* self.data[ self.model.name ] */ )
    //             .css({
    //                 'width': '100%'
    //             })
    //             .keyup( function ( event ) {
    ...
    //     );
    this.element = `
    <p style="margin: 2%;padding: 2%; font-size: 14px;">
    This app does not store any location data.  
    It sets up a virtual “perimeter” and then sends a notification to the device when the device is inside the perimeter.  
    The actual “tracking” is happening at the host of the geofence which in this case is some Google server.
    </p>
    <p style="margin: 2%;padding: 2%; font-size: 14px;">
    In order to receive the promotional messages that have been described in the previous page, 
    we will need you to make sure that the settings on your device allows this app to use your location information in the background.  
    Pressing the “Accept” button below will bring you to the settings page where you can allow the app to give you this feature.
    </p>
    `;
}
...
function populateOptions() {
    ...
    var proxidemo = Object();
    proxidemo.value = "mcba://proxidemo";
    proxidemo.text = "Geo";
    options.push( proxidemo );
...
function ActionItem( item ) {  // MARK: Action Item
    var self = this;
    this.item = item;
    self[ "model" ] = Object();         // create the model property
    self[ "model" ].name = "action";    // give the new property a name property of "action"
    // set pointer to mockup element
    // if ( item.item.element[ 0 ] != undefined && item.item.element[ 0 ] != null ) {
    //     action_pointer = item.item.element[ 0 ].children[ 0 ];
    // } else {
    //     console.log( "*** Warning: no children exist for " + item.item.element[ 0 ]);
    // }
    if ( item.item.children != null ) { // if this item has an id for the font picker, use it
        if ( item.item.children[ 6 ].id != undefined ) { self[ "id" ] = item.item.children[ 6 ].id; }
    } else { self[ "id" ] = "action_" + milliseconds_since_epoch(); }
    this.element = document.createElement( "div" ); // create div and input element.  assign id.
    var selectElement = document.createElement( "select" );
    selectElement.id = self[ "id" ];
    var options = populateOptions();

    // fork here depending on action
    // case left
    // case right
    // case single
    var current_aciton = this.item.item.item.left_button_action
    var remember_option_element = null;
    // probably need to move this to a different method..
    for (let index = 0; index < options.length; index++) {
        const value = options[index].value;
        const text = options[index].text;
        var optionElement = document.createElement("option");
        optionElement.value = value;
        optionElement.text = text;
        selectElement.appendChild(optionElement);  // Append first
        console.log( "item.data[ item.mode.name ] = " + item.data[ item.model.name ]);
        if ( item.data[ item.model.name ] == value) {
            optionElement.selected = true;
            selectElement.value = value;  // Explicitly set select value
            remember_option_element = optionElement;
        }
    }

    if ( remember_option_element != null ) {  // remember this option and select it after load
        jQuery( document ).ready( function() { remember_option_element.selected = true; }); }

    selectElement.onchange = function(event) { // set onchange and include event parameter
        var actionValue = event.target[ event.target.selectedIndex ].value;
        console.log( "onchange fired by element type: " + event.target.tagName ); // Log the type of the element
        console.log( "changing " + self.item.item.item[ self.item.model.name ] + " to: " + actionValue );
        self.item.item.item[ self.item.model.name ] = actionValue;
    };
    this.initialize = function () {}
    var actionLabelDiv = document.createElement( "div" ); // create font label div 
    var actionLabel = document.createElement( "label" );  // append font label div
    actionLabel.innerHTML = self.item.model.display;
    actionLabelDiv.append( actionLabel );
    this.element.append( actionLabelDiv );
    this.element.append( selectElement );
    if ( self.item.item.item[ "action" ] != undefined ) { // set initial value
        selectElement.value = self.item.item.item[ "action" ]; }
}
// MARK: end Action Item
...
function FontItem( item ) {
    ...
    try {
        ...
        if ( button_text_pointer == undefined ) {
            button_text_pointer = item.item.element[ 0 ]; // just use the parent
        }
        ...
        console.error( "*** ERROR: error finding element in " + self.data[ self.model.name ]);
        return undefined;
    }
...
function PageItem( item ) {
    ...
    this.existsInWindow = function( item_to_check ) {
        let aka = item_to_check.replace( "Html",       "HTMLItem"   )
                               .replace( "Mcba_image", "MCBA_Image" );
        return window[ aka ];    
    }

    let ItemConstructor = undefined;
    this.refresh = function () {
        if( !this.existsInWindow( this.type )) {

            let thisItemObject = new( Function( 'return new ' + this.item.type.replace( "Html", "HTMLItem" ) ))( this.item );
            ItemConstructor = thisItemObject.constructor;
            console.log( "item object: " + this.type + " created with Function().")
        } else {
            ItemConstructor = window[ this.type.replace( "Html", "HTMLItem"         )
                                               .replace( "Mcba_image", "MCBA_Image" )];
        }
