...
function mcba_send_message($type, $message, $recipients, $latitude, $longitude, $radius, $expires, $direct) {
    ...
    if (!$direct) {
        if ($result = mysqli_query($db, 'SELECT * FROM ' . $wpdb->prefix . 'mcba_users')) {
            ...
            echo '<br>Problem: No results in ' . $wpdb->prefix . 'mcba_users' . "<br>";
    ...
    if ($result = mysqli_query($db, 'SELECT * FROM ' . $wpdb->prefix . 'mcba')) {
        ...
        echo ' Problem: No results in ' . $wpdb->prefix . 'mcba' . "<br>";
        exit();
    }
    mysqli_close($db);
    $args = array(
    ...
    $data = array(                         // make http promo blast call
				'message' => $message,
				'type' => $type,
				'radius' => $radius,
				'expires' => $expires,
    ...
    $notification = array('title' => "", 'body' => $message, 'sound' => 'default', 'badge' => '1',  'content-available' => '1');
