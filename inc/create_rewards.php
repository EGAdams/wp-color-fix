...
/*
  This file is not in use as of 2023.  It is just here to show another from of generating
  a QR code.  The codes don't work correctly so this file may just be deleted in the future.
*/
    ...
    if ( $_REQUEST['page'] == 'rwpm_send' && isset( $postSubmit  )) {
        ...
        if (empty( $content )) {
            ...
            $status[] = __('Please enter content of code.', 'pm4wp');
                ...
                $logger->logUpdate( "generating code for content: " . $content );
                ...
                if ( $result['error_code'] === -1) {
                    ...
                    if (!empty( $content )) {
						$logger->logUpdate( "adding content: " . $content );
                        $image1 = "image1.png";
                        QRcode::png( $dataToEncode, $image1, 'L', 10, 10 );
                    }
                    if (!empty( $content2 )) {
						$logger->logUpdate( "adding content: " . $content2 );
                        $dataToEncode = $result['gcm_id'] . "MCBA" . $content2;
                        $image2 = "image2.png";
                        QRcode::png( $dataToEncode, $image2, 'L', 10, 10 );
                    }
                    if (!empty( $content3 )) {
						$logger->logUpdate( "adding content: " . $content3 );
                        $dataToEncode = $result['gcm_id'] . "MCBA" . $content3;
                        $image3 = "image3.png";
                        QRcode::png( $dataToEncode, $image3, 'L', 10, 10 );
                    }
                    if (!empty( $content4 )) {
						$logger->logUpdate( "adding content: " . $content4 );
                        $dataToEncode = $result['gcm_id'] . "MCBA" . $content4;
                        $image4 = "image4.png";
                        QRcode::png( $dataToEncode, $image4, 'L', 10, 10 );
                    }
                }
        }
    }
